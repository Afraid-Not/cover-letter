...
- **회사 정보 조사/직접 입력** — 프로젝트 생성 시 OpenAI web search로 미션/비전/주요 제품·서비스 자동 수집; 검색 결과 없을 시 직접 입력 가능, 생성 프롬프트에 컨텍스트로 주입
...
[회사 정보 조사]  →  OpenAI web search → 미션/비전/제품·서비스 (DB 저장, Step 1 표시)
...
│   ├── researcher.py         # 회사명 → OpenAI web search → 미션/비전/제품·서비스
│   ├── retriever.py          # 벡터 유사도 검색
│   ├── generator.py          # RAG + 자소서 생성 (회사 정보 컨텍스트 포함)
