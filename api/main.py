...
async def _get_user_plan(sb, user_id: str) -> str:
    try:
        result = await _run(
            lambda: sb.table("profiles").select("plan").eq("user_id", user_id).single().execute()
        )
...
async def _count_monthly_usage(sb, user_id: str) -> dict:
    month_start = _get_month_start()
    try:
        gen_res, regen_res = await asyncio.gather(
            _run(lambda: (
...
async def _count_resumes(sb, user_id: str) -> int:
    try:
        res = await _run(lambda: sb.table("resumes").select("id", count="exact").eq("user_id", user_id).execute())
...
async def analyze_job(req: AnalyzeJobRequest):
    ...
    return await analyze_job_posting(req.text)
...
async def generate(req: GenerateRequest, request: Request):
    ...
    if token:
        ...
        if gen_user_id:
            ...
            plan = await _get_user_plan(sb_plan, gen_user_id)
            ...
            if is_regen:
                ...
                try:
                    extra_res = await _run(lambda: sb_plan.table("profiles").select("extra_regenerations").eq("user_id", gen_user_id).single().execute())
                ...
                if effective_regen_limit == 0:
                    ...
                    monthly = await _count_monthly_usage(sb_plan, gen_user_id)
                ...
                if limits["generations"] > 0:
                    try:
                        extra_gen_res = await _run(lambda: sb_plan.table("profiles").select("extra_generations").eq("user_id", gen_user_id).single().execute())
    ...
    job_analysis = req.job_analysis if req.job_analysis else await analyze_job_posting(req.job_posting)

    # 이력서 resolve
    if req.resume_id:
        resume_data = await get_resume(req.resume_id)
        structured = resume_data["structured_data"]
        raw_text = resume_data.get("raw_text", "")
    elif req.resume_text:
        result = await process_resume(req.resume_text)
    ...
    rag_results = await search_similar(search_query, match_count=5)
...
async def evaluate(req: EvaluateRequest, request: Request):
    ...
    if token:
        ...
        if user_id:
            ...
            plan = await _get_user_plan(sb_eval, user_id)
            if plan == "free":
                try:
                    extra_res = await _run(lambda: sb_eval.table("profiles").select("extra_regenerations").eq("user_id", user_id).single().execute())
    ...
    if token:
        ...
        if uid:
            try:
                sb_bd = _get_sb(token)
                bd_res = await _run(lambda: sb_bd.table("profiles").select("birth_date").eq("user_id", uid).single().execute())
        ...
        if user_id:
            ...
            if plan == "free":
                ...
                if extra_regen <= 0:
                    raise HTTPException(status_code=403, detail="PLAN_LIMIT|평가 기능은 Pro 플랜 이상에서 사용할 수 있습니다.")
            try:
                bd_res = await _run(lambda: sb_eval.table("profiles").select("birth_date").eq("user_id", user_id).single().execute())
...
async def list_resumes(request: Request):
    ...
    try:
        return await list_resumes(user_id=user_id)
    except Exception as e:
...
async def add_resume(req: ResumeRequest, request: Request):
    ...
    if user_id:
        ...
        plan = await _get_user_plan(sb_check, user_id)
        limits = PLAN_LIMITS[plan]
        if limits["resumes"] > 0:
            count = await _count_resumes(sb_check, user_id)
            if count >= limits["resumes"]:
                raise HTTPException(status_code=403, detail="PLAN_LIMIT|이력서 등록 한도에 도달했습니다.")
    try:
        return await process_resume(req.source, req.name, user_id=user_id)
    except Exception as e:
    ...
    try:
        if suffix == "pdf":
            with tempfile.NamedTemporaryFile(delete=False, suffix=".pdf") as tmp:
                tmp.write(content)
                tmp_path = tmp.name
            result = await process_resume(tmp_path, name or file.filename, user_id=user_id)
            import os
            os.unlink(tmp_path)
        else:
            text = content.decode("utf-8")
            result = await process_resume(text, name or file.filename, user_id=user_id)
    except Exception as e:
...
async def update_resume(resume_id: int, body: dict, request: Request):
    ...
    try:
        result = await _run(lambda: sb.table("resumes").update(update_fields).eq("id", resume_id).execute())
...
async def list_generations():
    ...
    try:
        result = await _run(lambda: sb.table("generations").select(
...
async def save_generation(req: SaveGenerationRequest):
    ...
    sb = create_client(os.getenv("SUPABASE_URL"), os.getenv("SUPABASE_KEY"))
    try:
        result = await _run(lambda: sb.table("generations").insert({
...
async def get_generation(gen_id: int):
    ...
    sb = create_client(os.getenv("SUPABASE_URL"), os.getenv("SUPABASE_KEY"))
    try:
        result = await _run(lambda: sb.table("generations").select("*").eq("id", gen_id).single().execute())
    except Exception as e:
...
async def create_project(req: CreateProjectRequest, request: Request):
    ...
    from src.analyzer import analyze_job_posting
    job_analysis = await analyze_job_posting(req.job_posting)
    ...
    try:
        result = await _run(lambda: sb.table("generations").insert(insert_data).execute())
    except Exception as e:
...
async def research_project_company(project_id: int, request: Request):
    ...
    user_id = _get_user_id(token) if token else None
    if user_id:
        plan = await _get_user_plan(sb, user_id)
        if plan == "free":
            sb_admin = _get_sb()
            try:
                extra_res = await _run(lambda: sb_admin.table("profiles").select("extra_company_searches").eq("user_id", user_id).single().execute())
    ...
    project_result = await _run(lambda: sb.table("generations").select("job_analysis, company_id").eq("id", project_id).single().execute())
    ...
    company_row = await get_or_research_company(company)
    logging.getLogger(__name__).info("[research] company='%s' row=%s", company, company_row)

    if company_row:
        try:
            await _run(lambda: sb.table("generations").update({"company_id": company_row["id"]}).eq("id", project_id).execute())
        except Exception as e:
    ...
    if user_id and plan == "free":  # type: ignore[possibly-undefined]
        try:
            await _run(lambda: sb_admin.table("profiles").update({"extra_company_searches": extra_searches - 1}).eq("user_id", user_id).execute())  # type: ignore[possibly-undefined]
        except Exception:
            pass  # 차감 실패는 무시 (이미 검색은 완료됨)

    # 최신 프로젝트 데이터를 companies 조인해서 반환
    result = await _run(lambda: sb.table("generations").select("*, companies(*)").eq("id", project_id).single().execute())
    return result.data
...
async def list_activity(request: Request):
    ...
    results = await asyncio.gather(
        _run(lambda: sb.table("generations").select(
        ...
        _run(lambda: sb.table("resumes").select(
            "id, name, created_at"
    ...
    # 재생성 (versions) — RLS가 user_id 필터링해줌
    if not isinstance(results[2], Exception):
        for r in (results[2].data or []):
            ja = r.get("job_analysis") or {}
            events.append({
                "type": "regeneration",
                "id": r["project_id"],
...
async def get_project(project_id: int, request: Request):
    ...
    sb = _get_sb(token)
    try:
        result, versions = await asyncio.gather(
            _run(lambda: sb.table("generations").select("*, companies(*)").eq("id", project_id).single().execute()),
            _run(lambda: sb.table("generations").select(
...
async def update_project(project_id: int, req: UpdateProjectRequest, request: Request):
    ...
    if company_research:
        proj = await _run(lambda: sb.table("generations").select("company_id").eq("id", project_id).single().execute())
        ...
        if company_id:
            ...
            await _run(lambda: _supabase_admin.table("companies").update(company_research).eq("id", company_id).execute())
        else:
            # company_id 없으면 research 엔드포인트처럼 새로 생성
            from src.researcher import _save_company, _normalize, _get_embedding
            gen_res = await _run(lambda: sb.table("generations").select("job_analysis").eq("id", project_id).single().execute())
            company_name = (gen_res.data or {}).get("job_analysis", {}).get("company", "")
            if company_name:
                row = await _save_company(company_name, company_research)
                await _run(lambda: sb.table("generations").update({"company_id": row["id"]}).eq("id", project_id).execute())

    update_data = {k: v for k, v in data.items() if v is not None}
    if update_data:
        try:
            result = await _run(lambda: sb.table("generations").update(update_data).eq("id", project_id).execute())
        except Exception as e:
            raise HTTPException(status_code=500, detail=f"DB update failed: {e}")
        if not result.data:
...
async def delete_project(project_id: int, request: Request):
    ...
    sb = _get_sb(token)
    try:
        await _run(lambda: sb.table("generations").update(
...
async def create_project_version(project_id: int, req: CreateVersionRequest, request: Request):
    ...
    # 루트 프로젝트 조회
    root_result = await _run(lambda: sb.table("generations").select("*").eq("id", project_id).single().execute())
    ...
    try:
        version_result = await _run(lambda: sb.table("generations").insert(version_data).execute())
        ...
        await _run(lambda: sb.table("generations").update(root_update).eq("id", project_id).execute())
    except Exception as e:
...
async def update_project_version(project_id: int, version_id: int, req: Request):
    ...
    try:
        # 버전 소유권 확인
        ver = await _run(lambda: sb.table("generations").select("user_id, project_id").eq("id", version_id).single().execute())
        if not ver.data:
            raise HTTPException(status_code=404, detail="버전을 찾을 수 없습니다")
        if user_id and ver.data.get("user_id") and ver.data["user_id"] != user_id:
            raise HTTPException(status_code=403, detail="권한이 없습니다")

        await _run(lambda: sb.table("generations").update({"evaluation": evaluation}).eq("id", version_id).execute())

        # 루트 행도 최신 evaluation 으로 업데이트
        await _run(lambda: sb.table("generations").update({"evaluation": evaluation}).eq("id", project_id).execute())
...
async def get_plan_settings():
    ...
    sb = _get_sb()
    try:
        result = await _run(lambda: sb.table("app_settings").select("key, value").in_(
...
async def get_usage(request: Request):
    ...
    monthly, resume_count = await asyncio.gather(
        _count_monthly_usage(sb, user_id),
        _count_resumes(sb, user_id),
    )

    # extra_regenerations / extra_generations / extra_company_searches 반영: 한도가 무제한(-1)이 아닌 경우에만 더함
    try:
        extra_res = await _run(lambda: sb.table("profiles").select("extra_regenerations, extra_generations, extra_company_searches").eq("user_id", user_id).single().execute())
        extra_regen = (extra_res.data or {}).get("extra_regenerations", 0) or 0
...
async def upsert_profile(req: ProfileRequest, request: Request):
    ...
    if req.job_title:
        ...
        response = await client.embeddings.create(model="text-embedding-3-small", input=req.job_title)
        profile_data["job_embedding"] = response.data[0].embedding

    sb = _get_sb(token)
    try:
        result = await _run(lambda: sb.table("profiles").upsert(profile_data, on_conflict="user_id").execute())
    except Exception as e:
...
async def change_plan(req: ChangePlanRequest, request: Request):
    ...
    sb_admin = _get_sb()
    try:
        setting = await _run(lambda: sb_admin.table("app_settings").select("value").eq("key", f"plan_{req.plan}_enabled").single().execute())
    ...
    sb = _get_sb(token)
    try:
        result = await _run(lambda: sb.table("profiles").update({"plan": req.plan}).eq("user_id", user_id).execute())
        if not result.data:
...
async def get_my_profile(request: Request):
    ...
    sb = _get_sb(token)
    try:
        result = await _run(lambda: sb.table("profiles").select("*").eq("user_id", user_id).single().execute())
    except Exception as e:
...
async def delete_account(request: Request):
    ...
    try:
        await _run(lambda: sb_admin.auth.admin.delete_user(user_id))
    except Exception as e:
...
async def _check_admin(token: str | None) -> str:
    ...
    try:
        result = await _run(lambda: sb.table("profiles").select("role").eq("user_id", user_id).single().execute())
...
async def admin_stats(request: Request):
    """서비스 전체 KPI 지표 조회."""
    await _check_admin(_extract_token(request))
    ...
    try:
        total_users_res, plan_dist_res, today_gen_res, total_gen_res = await asyncio.gather(
            _run(lambda: sb.table("profiles").select("user_id", count="exact").execute()),
            _run(lambda: sb.table("profiles").select("plan").execute()),
            _run(lambda: sb.table("generations").select("id", count="exact").gte("created_at", today_start).is_("project_id", "null").execute()),
            _run(lambda: sb.table("generations").select("id", count="exact").is_("project_id", "null").execute()),
        )
        ...
        today_gen = today_gen_res.count or 0
        total_gen = total_gen_res.count or 0
    except Exception as e:
...
async def admin_list_users(request: Request):
    ...
    try:
        profiles_res = await _run(lambda: sb.table("profiles").select("user_id, name, plan, role, extra_regenerations, extra_company_searches, created_at").execute())
...
async def admin_list_resumes(request: Request):
    ...
    sb = _get_sb()
    try:
        result = await _run(lambda: sb.table("resumes").select(
...
async def admin_get_settings(request: Request):
    ...
    sb = _get_sb()
    try:
        result = await _run(lambda: sb.table("app_settings").select("key, value").execute())
...
async def admin_update_settings(req: AdminSettingsRequest, request: Request):
    ...
    try:
        for key, val in updates.items():
            await _run(lambda k=key, v=val: sb.table("app_settings").upsert({"key": k, "value": v, "updated_at": "now()"}, on_conflict="key").execute())
    except Exception as e:
...
async def admin_change_user_plan(user_id: str, req: AdminChangePlanRequest, request: Request):
    ...
    sb = _get_sb()
    try:
        await _run(lambda: sb.table("profiles").update({"plan": req.plan}).eq("user_id", user_id).execute())
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"플랜 변경 실패: {e}")
    return {"ok": True}
...
async def admin_set_extra_regenerations(user_id: str, req: AdminExtraRegenRequest, request: Request):
    ...
    sb = _get_sb()
    try:
        await _run(lambda: sb.table("profiles").update({"extra_regenerations": req.extra_regenerations}).eq("user_id", user_id).execute())
    except Exception as e:
...
async def admin_set_extra_company_searches(user_id: str, req: AdminExtraCompanySearchRequest, request: Request):
    ...
    sb = _get_sb()
    try:
        await _run(lambda: sb.table("profiles").update({"extra_company_searches": req.extra_company_searches}).eq("user_id", user_id).execute())
    except Exception as e:
...
async def admin_create_coupon(req: AdminCouponCreateRequest, request: Request):
    ...
    sb = _get_sb()
    try:
        await _run(lambda: sb.table("coupons").insert({
            "code": code,
...
async def admin_list_coupons(request: Request):
    ...
    sb = _get_sb()
    try:
        res = await _run(lambda: sb.table("coupons").select("*").order("created_at", desc=True).execute())
        return res.data or []
    except Exception as e:
...
async def redeem_coupon(req: CouponRedeemRequest, request: Request):
    ...
    plan = await _get_user_plan(sb_user, user_id)
    if plan == "enterprise":
    ...
    try:
        coupon_res = await _run(lambda: sb.table("coupons").select("*").eq("code", code).single().execute())
    except Exception:
    ...
    # 쿠폰 사용 처리
    try:
        await _run(lambda: sb.table("coupons").update({"used_by": user_id, "used_at": now}).eq("code", code).execute())
    except Exception as e:
    ...
    try:
        profile_res = await _run(lambda: sb.table("profiles").select("extra_generations, extra_regenerations, extra_company_searches").eq("user_id", user_id).single().execute())
        ...
        await _run(lambda: sb.table("profiles").update({
...
async def billing_auth(req: BillingAuthRequest, request: Request):
    ...
    sb = _get_sb()  # service_role — RLS bypass
    await _run(lambda: sb.table("subscriptions").upsert(
        {
            "user_id": user_id,
            "plan": req.plan,
...
async def cancel_subscription(request: Request):
    ...
    now = datetime.now(timezone.utc).isoformat()
    sb = _get_sb()
    await _run(lambda: sb.table("subscriptions").update({"status": "cancelled", "updated_at": now}).eq("user_id", user_id).execute())
    await _run(lambda: sb.table("profiles").update({"plan": "free"}).eq("user_id", user_id).execute())
    return {"ok": True}
...
async def payment_webhook(request: Request):
    ...
    if status == "DONE":
        next_billing = now + timedelta(days=30)
        await _run(lambda: sb.table("subscriptions").update(
            {
                "status": "active",
                "last_billed_at": now.isoformat(),
                "next_billing_date": next_billing.isoformat(),
        ...
        sub = await _run(lambda: sb.table("subscriptions").select("user_id").eq("customer_key", customer_key).single().execute())
        if sub.data:
            uid = sub.data["user_id"]
            await _run(lambda: sb.table("subscriptions").update({"status": "failed", "updated_at": now.isoformat()}).eq("customer_key", customer_key).execute())
            await _run(lambda: sb.table("profiles").update({"plan": "free"}).eq("user_id", uid).execute())
    ...
    async with httpx.AsyncClient(timeout=15.0) as client:
        for sub in subs.data:
            ...
            try:
                ...
                await _run(lambda: sb.table("subscriptions").update({"status": "failed", "updated_at": now.isoformat()}).eq("user_id", sub["user_id"]).execute())
                await _run(lambda: sb.table("profiles").update({"plan": "free"}).eq("user_id", sub["user_id"]).execute())
                failed += 1
